module Mcp::Tools
  ...
  private
    ...
    def create_board(name:, columns: nil)
      ...
      board = Current.user.account.boards.create!(name: name, creator: Current.user)
      columns.each { |col| board.columns.create!(name: col) }
      board.accesses.grant_to(Current.user)
    ...
    def board_summary(board)
      {
        ...
        url: Rails.application.routes.url_helpers.board_url(board, host: default_host)
      }
    end
    ...
    def card_summary(card)
      {
        ...
        url: Rails.application.routes.url_helpers.card_url(card, host: default_host)
      }
    end
